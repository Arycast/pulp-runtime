...
#define START_WRITE_ADDRESS    0x00000800
#endif
...
int main(void)
{
	...
	uint8_t         read_after_write_buffer[BUFFER_SIZE], read_after_erase_buffer[BUFFER_SIZE];
	...
	/* check if requested address is beyond flash size */
	if ((base_address + BUFFER_SIZE) >= SPI_FLASH_SIZE_BYTES)
	{
		printf("requested space for test is beyond flash size: " \
			"address: 0x%08x, size: %u sector_offset_bytes" LINE_END,
			(unsigned int) base_address, (unsigned int) BUFFER_SIZE);
	...
	puts("Finished setting up CS pin");
	...
	/**
		* **************************************************
		* write new data and read new data
		*/
		...
		++i, ++address, ++address_within_sector)
	{
		printf("Address: 0x%08x | Value Before: 0x%x | Value After: 0x%x | TEST: 0x%x" LINE_END,
			...
			(unsigned int) read_after_erase_buffer[i],
